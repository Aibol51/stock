...
func (aq *AccountQuery) QueryViews() *ViewQuery {
	query := (&ViewClient{config: aq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := aq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (aq *AccountQuery) WithViews(opts ...func(*ViewQuery)) *AccountQuery {
	query := (&ViewClient{config: aq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (aq *AccountQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Account, error) {
	...
	if query := aq.withViews; query != nil {
		if err := aq.loadViews(ctx, query, nodes,
			func(n *Account) { n.Edges.Views = []*View{} },
			func(n *Account, e *View) { n.Edges.Views = append(n.Edges.Views, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (aq *AccountQuery) loadComments(ctx context.Context, query *CommentQuery, nodes []*Account, init func(*Account), assign func(*Account, *Comment)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(account.CommentsTable)
		s.Join(joinT).On(s.C(comment.FieldID), joinT.C(account.CommentsPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(account.CommentsPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(account.CommentsPrimaryKey[0]))
...
func (aq *AccountQuery) loadLikes(ctx context.Context, query *LikeQuery, nodes []*Account, init func(*Account), assign func(*Account, *Like)) error {
	...
	for _, n := range neighbors {
		...
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "account_likes" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (aq *AccountQuery) loadViews(ctx context.Context, query *ViewQuery, nodes []*Account, init func(*Account), assign func(*Account, *View)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Account)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.View(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(account.ViewsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.account_views
		if fk == nil {
			return fmt.Errorf(`foreign-key "account_views" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "account_views" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
