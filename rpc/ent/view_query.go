...
func (vq *ViewQuery) QueryAccount() *AccountQuery {
...
func (vq *ViewQuery) QueryPost() *PostQuery {
...
func (vq *ViewQuery) Clone() *ViewQuery {
	...
	return &ViewQuery{
		...
		withAccount: vq.withAccount.Clone(),
...
func (vq *ViewQuery) WithAccount(opts ...func(*AccountQuery)) *ViewQuery {
	query := (&AccountClient{config: vq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	vq.withAccount = query
	return vq
}
...
func (vq *ViewQuery) WithPost(opts ...func(*PostQuery)) *ViewQuery {
	query := (&PostClient{config: vq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (vq *ViewQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*View, error) {
	...
	if query := vq.withAccount; query != nil {
		if err := vq.loadAccount(ctx, query, nodes, nil,
			func(n *View, e *Account) { n.Edges.Account = e }); err != nil {
			return nil, err
		}
	}
	if query := vq.withPost; query != nil {
		if err := vq.loadPost(ctx, query, nodes,
			func(n *View) { n.Edges.Post = []*Post{} },
			func(n *View, e *Post) { n.Edges.Post = append(n.Edges.Post, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (vq *ViewQuery) loadAccount(ctx context.Context, query *AccountQuery, nodes []*View, init func(*View), assign func(*View, *Account)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*View)
	for i := range nodes {
		if nodes[i].account_views == nil {
			continue
		}
		fk := *nodes[i].account_views
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "account_views" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (vq *ViewQuery) loadPost(ctx context.Context, query *PostQuery, nodes []*View, init func(*View), assign func(*View, *Post)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[uuid.UUID]*View)
	nids := make(map[uuid.UUID]map[*View]struct{})
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(view.PostTable)
		s.Join(joinT).On(s.C(post.FieldID), joinT.C(view.PostPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(view.PostPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(view.PostPrimaryKey[1]))
