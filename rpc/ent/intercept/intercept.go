...
type LikeFunc func(context.Context, *ent.LikeQuery) (ent.Value, error)

// Query calls f(ctx, q).
func (f LikeFunc) Query(ctx context.Context, q ent.Query) (ent.Value, error) {
	if q, ok := q.(*ent.LikeQuery); ok {
		return f(ctx, q)
	}
	return nil, fmt.Errorf("unexpected query type %T. expect *ent.LikeQuery", q)
}

// The TraverseLike type is an adapter to allow the use of ordinary function as Traverser.
type TraverseLike func(context.Context, *ent.LikeQuery) error

// Intercept is a dummy implementation of Intercept that returns the next Querier in the pipeline.
func (f TraverseLike) Intercept(next ent.Querier) ent.Querier {
	return next
}

// Traverse calls f(ctx, q).
func (f TraverseLike) Traverse(ctx context.Context, q ent.Query) error {
	if q, ok := q.(*ent.LikeQuery); ok {
		return f(ctx, q)
	}
	return fmt.Errorf("unexpected query type %T. expect *ent.LikeQuery", q)
}
...
type StockFunc func(context.Context, *ent.StockQuery) (ent.Value, error)

// Query calls f(ctx, q).
func (f StockFunc) Query(ctx context.Context, q ent.Query) (ent.Value, error) {
	if q, ok := q.(*ent.StockQuery); ok {
		return f(ctx, q)
	}
	return nil, fmt.Errorf("unexpected query type %T. expect *ent.StockQuery", q)
}

// The TraverseStock type is an adapter to allow the use of ordinary function as Traverser.
type TraverseStock func(context.Context, *ent.StockQuery) error

// Intercept is a dummy implementation of Intercept that returns the next Querier in the pipeline.
func (f TraverseStock) Intercept(next ent.Querier) ent.Querier {
	return next
}

// Traverse calls f(ctx, q).
func (f TraverseStock) Traverse(ctx context.Context, q ent.Query) error {
	if q, ok := q.(*ent.StockQuery); ok {
		return f(ctx, q)
	}
...
func NewQuery(q ent.Query) (Query, error) {
	...
	case *ent.LikeQuery:
		return &query[*ent.LikeQuery, predicate.Like, like.OrderOption]{typ: ent.TypeLike, tq: q}, nil
	case *ent.MenuQuery:
