...
func (lq *LikeQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Like, error) {
	...
	if query := lq.withPost; query != nil {
		if err := lq.loadPost(ctx, query, nodes,
			func(n *Like) { n.Edges.Post = []*Post{} },
			func(n *Like, e *Post) { n.Edges.Post = append(n.Edges.Post, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (lq *LikeQuery) loadAccount(ctx context.Context, query *AccountQuery, nodes []*Like, init func(*Like), assign func(*Like, *Account)) error {
	...
	for i := range nodes {
		if nodes[i].account_likes == nil {
			continue
		}
		fk := *nodes[i].account_likes
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "account_likes" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (lq *LikeQuery) loadPost(ctx context.Context, query *PostQuery, nodes []*Like, init func(*Like), assign func(*Like, *Post)) error {
	...
	byID := make(map[uuid.UUID]*Like)
	nids := make(map[uuid.UUID]map[*Like]struct{})
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(like.PostTable)
		s.Join(joinT).On(s.C(post.FieldID), joinT.C(like.PostPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(like.PostPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(like.PostPrimaryKey[1]))
