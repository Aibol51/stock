...
func (pq *PostQuery) QueryViews() *ViewQuery {
	query := (&ViewClient{config: pq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := pq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (pq *PostQuery) WithViews(opts ...func(*ViewQuery)) *PostQuery {
	query := (&ViewClient{config: pq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (pq *PostQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Post, error) {
	...
	if query := pq.withViews; query != nil {
		if err := pq.loadViews(ctx, query, nodes,
			func(n *Post) { n.Edges.Views = []*View{} },
			func(n *Post, e *View) { n.Edges.Views = append(n.Edges.Views, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (pq *PostQuery) loadLikes(ctx context.Context, query *LikeQuery, nodes []*Post, init func(*Post), assign func(*Post, *Like)) error {
	...
	neighbors, err := withInterceptors[[]*Like](ctx, query, qr, query.inters)
	if err != nil {
		return err
	}
	for _, n := range neighbors {
...
func (pq *PostQuery) loadViews(ctx context.Context, query *ViewQuery, nodes []*Post, init func(*Post), assign func(*Post, *View)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[uuid.UUID]*Post)
	nids := make(map[uuid.UUID]map[*Post]struct{})
	for i, node := range nodes {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(post.ViewsTable)
		s.Join(joinT).On(s.C(view.FieldID), joinT.C(post.ViewsPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(post.ViewsPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(post.ViewsPrimaryKey[0]))
		s.AppendSelect(columns...)
		s.SetDistinct(false)
	})
	if err := query.prepareQuery(ctx); err != nil {
