...
func (cq *CommentQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Comment, error) {
	...
	if query := cq.withAccount; query != nil {
		if err := cq.loadAccount(ctx, query, nodes,
			func(n *Comment) { n.Edges.Account = []*Account{} },
			func(n *Comment, e *Account) { n.Edges.Account = append(n.Edges.Account, e) }); err != nil {
			return nil, err
		}
	}
...
func (cq *CommentQuery) loadAccount(ctx context.Context, query *AccountQuery, nodes []*Comment, init func(*Comment), assign func(*Comment, *Account)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(comment.AccountTable)
		s.Join(joinT).On(s.C(account.FieldID), joinT.C(comment.AccountPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(comment.AccountPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(comment.AccountPrimaryKey[1]))
