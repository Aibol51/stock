...
func (l *LoginByEmailLogic) LoginByEmail(req *types.StockUserLoginByEmailReq) (resp *types.StockUserLoginResp, err error) {
	...
	if captchaData == req.Captcha {
		...
		token, err := jwt.NewJwtToken(l.svcCtx.Config.Auth.AccessSecret, time.Now().Unix(),
			l.svcCtx.Config.Auth.AccessExpire, jwt.WithOption("userId", userData.Data[0].Id))
		if err != nil {
			return nil, err
		}
