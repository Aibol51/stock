...
func (l *LoginByMobileLogic) LoginByMobile(req *types.StockUserLoginByMobileReq) (resp *types.StockUserLoginResp, err error) {
	...
	if ok := l.svcCtx.Captcha.Verify("CAPTCHA_"+req.CaptchaId, req.Captcha, true); ok {
		user, err := l.svcCtx.CoreRpc.GetStockUserByMobile(l.ctx,
			&core.StockUserMobileReq{
