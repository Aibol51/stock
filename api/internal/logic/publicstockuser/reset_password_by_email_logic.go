...
func (l *ResetPasswordByEmailLogic) ResetPasswordByEmail(req *types.StockUserResetPasswordByEmailReq) (resp *types.BaseMsgResp, err error) {
	...
	if captchaData == req.Captcha {
		userData, err := l.svcCtx.CoreRpc.GetStockUserList(l.ctx, &core.StockUserListReq{
			Page:     1,
			PageSize: 1,
			Email:    &req.Email,
		})
		...
		result, err := l.svcCtx.CoreRpc.UpdateStockUser(l.ctx, &core.StockUserInfo{Id: userData.Data[0].Id, Password: &req.Password})
		if err != nil {
			return nil, err
		}
